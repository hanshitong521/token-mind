...
describe("install", () => {
	it("installs hooks, library and assets and exits 0", () => {
		...
		for (const file of ["cm-lib.mjs", "cm-pre-tool.mjs", "cm-post-tool.mjs", "cm-session-start.mjs"]) {
	...
	it("writes a hooks.json that Cursor can consume", () => {
		...
		// Two preToolUse entries share one command; an earlier version of the
		// merge filtered them against each other and silently dropped Read and
		// Shell governance. This is the regression test for that.
		const pre = (hooks.hooks.preToolUse ?? []).filter((h) => /cm-pre-tool/.test(String(h.command)));
		assert.equal(pre.length, 2, `expected Read/Grep/Glob/Shell and Task entries, got ${JSON.stringify(pre)}`);
		assert.ok(pre.some((h) => String(h.matcher ?? "").includes("Read")));
	...
	it("retires predecessor gate hooks instead of governing twice", () => {
		...
		assert.ok(after.hooks.postToolUse.some((h) => String(h.command).includes("cm-post-tool")));
	});
...
describe("uninstall", () => {
	it("removes what it added and leaves the user's hooks alone", () => {
		...
		for (const list of Object.values(hooks.hooks)) {
			for (const h of list) assert.doesNotMatch(String(h.command), /hooks\/cm-/);
		}
	});
