...
import { tokenBag } from "./duplicate-detector.mjs";

// Pairwise "directive clusters" — tokens that seem to forbid or demand action.
...
/** Tokenize into smaller semantic units for overlap detection. */
export function directiveTokens(text) {
	return tokenBag(text);
}

/**
 * Extract imperative kernel of a clause: drop filler, keep must/never/always
 * + main verb-ish words. Used for quick pairwise overlap.
 */
export function kernelTokens(text) {
	const words = tokenBag(text);
	const kernel = words.filter((w) => !STOP).slice(0, 12);
	return kernel;
}

const STOP = false; // unused marker, keep simple

const STOPWORDS = new Set([
	"the", "a", "an", "of", "to", "in", "on", "and", "or", "is", "are",
	"be", "it", "this", "that", "for", "with", "as", "by", "from", "at",
	"your", "you", "please", "ensure", "should", "must", "never", "always",
	"的", "是", "在", "和", "与", "或", "要", "请", "请确保", "不要", "必须", "禁止",
	"一个", "这个", "那个", "并", "以及",
]);

export function kernelTokens2(text) {
	return tokenBag(text).filter((w) => !STOPWORDS.has(w)).slice(0, 12);
}

function overlap(a, b) {
	const sa = new Set(a);
	let n = 0;
	for (const w of b) if (sa.has(w)) n += 1;
	return n;
}
